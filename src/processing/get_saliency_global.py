...
def main() -> None:
    ...
    for experiment_id in fixation_data["ExperimentId"].unique():
        ...
        for sequence_id in tqdm(
            experiment_data["SequenceId"].unique(),
            desc=f"⌛ Computing KDE density for experiment {experiment_id}...",
            unit="sequence",
        ):
            sequence_data = experiment_data[
                experiment_data["SequenceId"] == sequence_id
            ]
            ...
            saliency_map_pfm_path = (
                f"{SALIENCY_MAP_PFM_PATH}/e{experiment_id}_sq{sequence_id}.pfm"
            )
            os.makedirs(SALIENCY_MAP_PFM_PATH, exist_ok=True)
            justpfm.write_pfm(file_name=saliency_map_pfm_path, data=kde_density)
            saliency_map_img_path = (
                f"{SALIENCY_MAP_IMG_PATH}/e{experiment_id}_sq{sequence_id}.png"
            )
            kde_density = cv2.normalize(
                kde_density, None, 0, 255, cv2.NORM_MINMAX
            ).astype(np.uint8)
            os.makedirs(SALIENCY_MAP_IMG_PATH, exist_ok=True)
